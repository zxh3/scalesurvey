...
export async function saveSurvey(survey: Omit<LocalSurvey, 'id' | 'lastAccessedAt'>): Promise<number> {
...
export async function getSurveyByKey(key: string): Promise<LocalSurvey | undefined> {
  return await db.surveys.where('key').equals(key).first();
}

export async function getSurveyBySurveyId(surveyId: string): Promise<LocalSurvey | undefined> {
