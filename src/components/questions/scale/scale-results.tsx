...
import { QuestionResultsProps, ScaleConfig } from "@/types/questions";
...
export function ScaleResults({ question, responses }: QuestionResultsProps) {
  ...
  const averageValue = totalResponses > 0
      ...
      <div className="flex items-center gap-4 p-4 bg-muted rounded-lg">
        ...
        <div className="flex-1">
          <div className="flex justify-between text-sm text-muted-foreground mb-1">
            <span>
              {config.minLabel || config.minValue}
            </span>
            <span>
              {config.maxLabel || config.maxValue}
            </span>
          </div>
