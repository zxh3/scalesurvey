...
export default function EditSurveyPage() {
  ...
  const initialData = useMemo(() => {
    ...
    const loadedQuestions: BaseQuestion[] = existingQuestions?.map((q) => ({
      _id: q._id,
      surveyId: q.surveyId,
      type: q.type as any,
      title: q.title,
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <Card className="max-w-md w-full">
          ...
          <CardContent>
            ...
            <Button className="w-full" onClick={() => router.push(`/admin/${adminCode}`)}>
              Back to Admin Panel
            </Button>
          </CardContent>
        </Card>
  ...
  const handleSave = async (data: SurveyFormData) => {
    ...
    try {
      ...
      const currentQuestionIds = new Set(
        data.questions.filter((q) => isExistingQuestion(q._id)).map((q) => q._id)
      );
