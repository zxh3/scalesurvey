...
import { ArrowLeft, Download, AlertCircle, XCircle } from "lucide-react";
...
export default function ResultsPage() {
  ...
  if (survey === undefined || questions === undefined || responses === undefined) {
    return (
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <Card className="max-w-md w-full">
          ...
          <CardContent>
            ...
            <Button className="mt-4 w-full" onClick={() => router.push("/access")}>
              Try Again
            </Button>
          </CardContent>
        </Card>
  ...
  const handleExportCSV = () => {
    // Create CSV header
    const headers = ["Response ID", "Submitted At", ...parsedQuestions.map((q) => q.title)];
    ...
    const rows = parsedResponses.map((response, index) => {
      const row = [
        ...
        ...parsedQuestions.map((question) => {
          const answer = response.answers.find((a: any) => a.questionId === question._id);
          if (!answer) return "";
      ...
      <div className="container max-w-5xl mx-auto">
        ...
        {responses.length === 0 && (
          <Alert>
            ...
            <AlertDescription>
              No responses yet. Share your survey URL to start collecting responses.
            </AlertDescription>
          </Alert>
        )}
        ...
        {responses.length > 0 && (
          <div className="space-y-6">
            {responsesByQuestion.map(({ question, responses: questionResponses }) => {
              ...
              return (
                <Card key={question._id}>
                  <CardContent className="pt-6">
                    <ResultsComponent question={question} responses={questionResponses} />
                  </CardContent>
                </Card>
              );
            })}
